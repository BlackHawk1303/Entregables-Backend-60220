...
export default class ProdServices {
    ...
    getProducts = async (page, limit, find, url, ord) => {
        ...
        const prevLink = item.hasPrevPage ? `${hostname.protocol}://${hostname.host}/products?page=${item.prevPage}` : ''
        const nextLink = item.hasNextPage ? `${hostname.protocol}://${hostname.host}/products?page=${item.nextPage}` : ''
